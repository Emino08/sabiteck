...
const ScholarshipDetail = () => {
    ...
    // Helper to recursively normalize IDs from various nested formats
    const normalizeIds = (input) => {
        ...
        if (typeof input === 'string') {
            ...
            try {
                ...
                // If parsing successful, recurse on the result
                return normalizeIds(parsed);
            } catch (e) {
                // If not valid JSON, treat as a single string ID (remove quotes if any)
        ...
        if (Array.isArray(input)) {
            // Recurse on each element and flatten
            return input.flatMap(item => normalizeIds(item));
        }
    ...
    const displayLevel = (level) => {
        ...
        const ids = normalizeIds(level);

        // Map IDs to names
        const names = ids.map(id => {
            const cleanId = String(id);
            const found = educationLevels.find(l => String(l.id) === cleanId || l.name === cleanId);
        ...
        return uniqueNames.length > 0 ? uniqueNames.join(', ') : 'All Levels';
    };

    const displayCategory = (category) => {
        if (!category) return 'Scholarship';

        const ids = normalizeIds(category);

        const names = ids.map(id => {
            const cleanId = String(id);
            const found = categories.find(c => String(c.id) === cleanId || c.name === cleanId);
            return found ? found.name : cleanId;
        }).filter(Boolean);

        const uniqueNames = [...new Set(names)];

        return uniqueNames.length > 0 ? uniqueNames.join(', ') : 'Scholarship';
    };
    ...
    const backendUrl = import.meta.env.VITE_API_URL || 'https://backend.sabiteck.com';

    // Generate proper OG image URL with sensible fallbacks
    const getFullImageUrl = (imageUrl) => {
        const placeholder = 'https://placehold.co/1200x630?text=Scholarship';
        if (!imageUrl || imageUrl.startsWith('data:')) return placeholder;

        // Use backend domain for uploaded assets
    ...
    const ogImage = getFullImageUrl(scholarship.image_url);
    const pageUrl = `/scholarships/${slug}`;
    const deadlineText = formatDeadline(scholarship.deadline);

    // Generate schema.org structured data for scholarships
    const scholarshipSchema = {
        "@context": "https://schema.org",
        "@type": "EducationalOccupationalProgram",
        "name": scholarship.title,
        "description": scholarship.short_description || scholarship.description,
        "provider": {
            "@type": "Organization",
            "name": scholarship.provider || "Sabiteck Limited"
        },
        "offers": {
            "@type": "Offer",
            "category": "Scholarship",
            "availabilityStarts": scholarship.created_at,
            "availabilityEnds": scholarship.deadline
        },
        "educationalLevel": scholarship.education_level,
    ...
    return (
        <>
            <SEOHead
                title={scholarship.title}
                description={scholarship.short_description || scholarship.description?.substring(0, 160) || `Apply for ${scholarship.title} scholarship. ${deadlineText}.`}
                keywords={`${scholarship.title}, scholarship, ${scholarship.category || 'education'}, ${scholarship.region || 'international'}, ${scholarship.education_level || 'study abroad'}, financial aid, education funding`}
                image={ogImage}
                url={pageUrl}
                type="article"
                    ...
                    <div className="grid lg:grid-cols-2 gap-8 items-center">
                        <div className="relative rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
                            ...
                            <div className="absolute bottom-4 left-4 flex items-center gap-3">
                                <div className="px-3 py-1 bg-black/60 text-white rounded-full text-xs font-bold border border-white/20">
                                    {displayCategory(scholarship.category) || 'Elite Scholarship'}
                                </div>
                        ...
                        <div className="bg-black/30 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <p className="text-sm uppercase tracking-wide text-yellow-300 font-semibold">Elite Scholarship</p>
                            </div>
                            <h1 className="text-3xl lg:text-4xl font-black text-white mb-3">{scholarship.title}</h1>
                            <p className="text-gray-200 mb-6">{scholarship.description || scholarship.short_description || 'Premium scholarship opportunity'}</p>
                            <div className="grid sm:grid-cols-2 gap-4">
                                <div className="flex items-center gap-3 p-3 bg-black/40 border border-white/10 rounded-2xl">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-bold text-green-300">${scholarship.amount || 'N/A'}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 p-3 bg-black/40 border border-white/10 rounded-2xl">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-bold text-red-300">{formatDeadline(scholarship.deadline)}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 p-3 bg-black/40 border border-white/10 rounded-2xl">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-bold text-blue-200">{displayLevel(scholarship.education_level)}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 p-3 bg-black/40 border border-white/10 rounded-2xl">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-bold text-purple-200">{scholarship.region || 'Global'}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="mt-6 flex gap-3">
                                <a
                                    ...
                                    className="flex-1 bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 text-white font-bold py-3 px-6 rounded-2xl text-center transition-all duration-300 hover:scale-105 hover:shadow-xl"
                                >
                                    Apply Now
                                </a>
                                <button
                                    onClick={handleSaveScholarship}
                                    className={`px-4 py-3 rounded-2xl font-bold border ${isSaved ? 'bg-emerald-500/20 border-emerald-400 text-emerald-100' : 'bg-black/40 border-white/20 text-white'} hover:scale-105 transition-all duration-300`}
                                >
                                    {isSaved ? 'Saved' : 'Save'}
                                </button>
                            </div>
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-indigo-500/20 to-purple-500/20 text-indigo-300 border border-indigo-500/30 backdrop-blur-lg">
                                                ...
                                                {displayCategory(scholarship.category)}
                                            </span>
                                            {scholarship.featured && (
                            ...
                            <div className="flex flex-col gap-4 lg:w-64">
                                ...
                                <div className="flex gap-3">
                                    <button
                                        onClick={handleSaveScholarship}
                                        className={`flex-1 px-4 py-3 rounded-2xl border font-bold transition-all duration-300 flex items-center justify-center gap-2 backdrop-blur-lg ${isSaved
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2">
                            ...
                            <div className="bg-black/30 backdrop-blur-xl rounded-3xl border border-white/10 p-8 mb-8 shadow-2xl hover:border-white/20 transition-all duration-500">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h2 className="text-2xl font-black text-white">About This Elite Opportunity</h2>
                                </div>
                                {scholarship.content ? (
                                    <div
                                        className="text-gray-300 prose prose-lg max-w-none prose-invert prose-indigo prose-headings:text-white prose-p:text-white prose-strong:text-white prose-li:text-white prose-ul:text-white prose-ol:text-white"
                            ...
                            <div className="bg-black/30 backdrop-blur-xl rounded-3xl border border-white/10 p-8 mb-8 shadow-2xl hover:border-white/20 transition-all duration-500">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div className={`flex items-center gap-4 p-4 rounded-2xl backdrop-blur-lg border transition-all duration-300 hover:scale-105 ${scholarship.covers_tuition ? 'bg-green-500/20 border-green-500/30' : 'bg-gray-500/20 border-gray-500/30'
                                        }`}>
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${scholarship.covers_tuition ? 'bg-green-500/30 border-green-500/50' : 'bg-gray-500/30 border-gray-500/50'
                                            }`}>
                                            <BookOpen className={`w-5 h-5 ${scholarship.covers_tuition ? 'text-green-400' : 'text-gray-400'
                                                }`} />
                                        </div>
                                        <div>
                                            <p className="font-bold text-white">Elite Tuition</p>
                                            <p className={`text-sm font-semibold ${scholarship.covers_tuition ? 'text-green-400' : 'text-gray-400'
                                                }`}>
                                    ...
                                    <div className={`flex items-center gap-4 p-4 rounded-2xl backdrop-blur-lg border transition-all duration-300 hover:scale-105 ${scholarship.covers_living ? 'bg-green-500/20 border-green-500/30' : 'bg-gray-500/20 border-gray-500/30'
                                        }`}>
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${scholarship.covers_living ? 'bg-green-500/30 border-green-500/50' : 'bg-gray-500/30 border-gray-500/50'
                                            }`}>
                                            <Users className={`w-5 h-5 ${scholarship.covers_living ? 'text-green-400' : 'text-gray-400'
                                                }`} />
                                        </div>
                                        <div>
                                            <p className="font-bold text-white">Elite Living</p>
                                            <p className={`text-sm font-semibold ${scholarship.covers_living ? 'text-green-400' : 'text-gray-400'
                                                }`}>
                                    ...
                                    <div className={`flex items-center gap-4 p-4 rounded-2xl backdrop-blur-lg border transition-all duration-300 hover:scale-105 ${scholarship.covers_travel ? 'bg-green-500/20 border-green-500/30' : 'bg-gray-500/20 border-gray-500/30'
                                        }`}>
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${scholarship.covers_travel ? 'bg-green-500/30 border-green-500/50' : 'bg-gray-500/30 border-gray-500/50'
                                            }`}>
                                            <MapPin className={`w-5 h-5 ${scholarship.covers_travel ? 'text-green-400' : 'text-gray-400'
                                                }`} />
                                        </div>
                                        <div>
                                            <p className="font-bold text-white">Elite Travel</p>
                                            <p className={`text-sm font-semibold ${scholarship.covers_travel ? 'text-green-400' : 'text-gray-400'
                                                }`}>
                        ...
                        <div className="space-y-8">
                            ...
                            {/* Elite Regions */}
                            {Array.isArray(scholarship.regions) && scholarship.regions.length > 0 && (
                                <div className="bg-black/30 backdrop-blur-xl rounded-3xl border border-white/10 p-6 shadow-2xl hover:border-white/20 transition-all duration-500">
                                    <div className="flex items-center gap-3 mb-6">
